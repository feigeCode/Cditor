...
>
> 架构约束：继续遵守 `doc/large-document-rich-text-architecture.md`。Cditor V2 仍然由 runtime 持有大文档真相，不能把 10w block 的数据真相交给 GPUI entity；但当前正在编辑的 block/cell/code-language 输入会话，必须像 ding-note2 一样有稳定、单一、可查询的输入状态。
>
