...
> 目标：把 Cditor 的表格从“能渲染的特殊 block”升级为接近 Notion 的完整表格编辑子系统。
>
> 更新规则：后续每完成一项，就把本文对应任务从 `[ ]` 改成 `[x]`，并在任务下方补测试、验收或风险说明。
>
> 架构约束：继续遵守 `doc/large-document-rich-text-architecture.md`。真实文档、表格数据、selection、layout height、scroll 坐标必须属于 runtime；GPUI 只负责当前窗口投影和交互事件。
...
- [x] H-014 表格视觉在 light theme 下接近 Notion。
  - 调整 `theme.border` 为 `0xe9e9e7`（Notion 风格的柔和灰色边框）、`table_header_background` 为 `0xf7f6f4`（Notion 风格的暖灰色表头）、`table_active_border` 为 `0x2383e2`（Notion 风格的蓝色活动边框）。
...
- [x] P-007 保存后重新打开，表格结构一致。
  - `postgres_integration_table_structure_survives_save_and_reopen` 覆盖 3x3 表格结构和单元格内容在保存/重新加载后保持一致。
- [x] P-008 保存后重新打开，row/column sizes 一致。
  - `postgres_integration_table_track_sizes_survive_save_and_reopen` 覆盖自定义行高（Px/Auto）和列宽（Px）在保存/重新加载后保持一致。
- [x] P-009 保存后重新打开，merge/align 一致。
  - `postgres_integration_table_merge_and_align_survive_save_and_reopen` 覆盖合并单元格元数据（Origin/Covered）和单元格对齐方式（Left/Center/Right）在保存/重新加载后保持一致。
- [x] P-010 保存后重新打开，layout cache 不导致表格高度错误。
  - `postgres_integration_table_height_cache_does_not_corrupt_after_reopen` 覆盖多行单元格内容和 layout cache 在保存/重新加载后不破坏表格 payload 结构。

### Q. Performance

- [x] Q-001 10w block 文档中表格 projection 不全量 layout 全文。
  - 现有架构：`payload_window` 只加载当前 viewport 附近 block；表格 block 独立进入 window，不触发全文 layout。`acceptance/open::tests::open_single_50k_row_table_uses_internal_virtualization_metadata` 覆盖大表打开使用虚拟化元数据而非全量加载。
- [x] Q-002 50k row table 使用虚拟化/分片策略，不一次渲染全部 cell。
  - 现有架构：`TableViewState.visible_cells` 仅投影当前 viewport 范围内的 cells；`large_table_projection_stays_within_viewport_budget` acceptance 覆盖 50k 行表格投影受 viewport row/cell budget 限制，不全量渲染。
- [x] Q-003 当前 viewport 表格 cell layout 控制在 frame budget 内。
  - 现有架构：visible cells 数量受 `viewport_cell_budget` (默认 960) 限制；cell text layout 使用 GPUI native text system，单帧 layout 成本在预算内。
- [x] Q-004 typing cell 时只更新当前 table/block 高度，不重算全局所有 block。
  - 现有架构：`refresh_table_block_height` 只触及当前 table block 的 `layout_meta` 和 `height_index` entry；`table_cell_typing_latency_touches_only_current_table_block` acceptance 覆盖 typing 只 touch 1 个 block。
...
- [x] Q-007 merge/split 大 range 有性能预算。
  - `table_merge_split_large_range_has_budget` acceptance 覆盖 64x8 range merge/split 在 24ms 预算内；实际实现中 merge/split 为 O(cells in range) 的 payload 操作，不扫描全表。
- [x] Q-008 增加 table typing latency acceptance。
  - `table_cell_typing_latency_touches_only_current_table_block` 覆盖 typing p95 < 8ms，且只 touch 当前 table block。
- [x] Q-009 增加 table resize drag frame budget acceptance。
  - `table_resize_drag_frame_budget_uses_preview_without_full_projection` 覆盖 resize drag p99 frame < 16ms，且使用 preview 状态不触发全量投影。
- [x] Q-010 增加 large table projection acceptance。
  - `large_table_projection_stays_within_viewport_budget` 覆盖 50k 行表格投影受 viewport budget 限制，不全量投影。

### R. GUI 验收

> **手动验收清单：** 详细的验收步骤和标准见 `doc/acceptance/table-manual-acceptance.md`，原型归档在 `doc/prototypes/notion-table-prototype.html`。

- [ ] R-001 插入 2x2 表格默认样式接近 Notion。
  - 验收要点：圆角边框、Notion 风格边框色、header row 暖灰色背景、合适的 padding。
- [ ] R-002 cell 内输入多行，表格高度增长，下面 block 下移。
  - 验收要点：单元格自动撑开、表格整体高度增加、下方段落下移、无重叠。
- [ ] R-003 cell 内中文 IME preview/commit 稳定。
  - 验收要点：IME 候选框正确定位、预览显示正确、提交后文字插入正确、多行场景光标跟随。
- [ ] R-004 row handle hover/selected/menu 正常。
  - 验收要点：hover 显示、selected 状态、gutter 主题色、菜单正确定位、菜单项完整。
- [ ] R-005 column handle hover/selected/menu 正常。
  - 验收要点：hover 显示、selected 状态、gutter 主题色、菜单正确定位、菜单项完整。
- [ ] R-006 resize column 视觉和结果正确。
  - 验收要点：resize cursor、实时预览线、拖动跟随、松手立即更新、支持 undo/redo。
- [ ] R-007 resize row 视觉和结果正确。
  - 验收要点：resize cursor、实时预览线、拖动跟随、松手立即更新、手动行高不被覆盖、支持 undo/redo。
- [ ] R-008 reorder row 视觉和结果正确。
  - 验收要点：drop indicator、插入位置正确、顺序更新、内容保留、merge metadata 重映射、支持 undo/redo。
- [ ] R-009 reorder column 视觉和结果正确。
  - 验收要点：drop indicator、插入位置正确、顺序更新、内容保留、merge metadata 重映射、支持 undo/redo。
- [ ] R-010 merge/split 视觉和结果正确。
  - 验收要点：range selection 连续背景、合并后 origin cell 显示内容、covered cells 不可见、拆分恢复独立、支持 undo/redo。
...
- [ ] R-012 active cell 与 editing caret 不冲突。
  - 验收要点：active cell 蓝色边框、光标正确显示、边框与光标不重叠、光标位置跟随输入、marked range 不冲突。
- [ ] R-013 表格在页面顶部/中间/底部都能正确更新高度。
  - 验收要点：顶部/中间/底部表格高度增长后滚动条和相邻 block 正确更新、无重叠。
- [ ] R-014 表格在滚动后编辑，candidate rect 不错位。
  - 验收要点：滚动后聚焦正确、IME 候选框位置正确、多行光标跟随、无闪烁跳跃。
...
- A 组（文档与基线）：5/5 ✅
- B 组（Engine 目录拆分）：13/13 ✅
- C 组（Table Runtime Invariant）：10/10 ✅
- D 组（TableLayout 引擎）：15/15 ✅
- E 组（Block Height 同步）：13/13 ✅
- F 组（Cell 输入与 IME）：15/15 ✅
- G 组（Selection 模型）：15/15 ✅
- H 组（GUI 样式）：15/15 ✅
- I 组（Row / Column Menu）：15/15 ✅
- J 组（Row / Column Resize）：15/15 ✅
- K 组（Row / Column Reorder）：15/15 ✅
- L 组（Merge / Split）：15/15 ✅
- M 组（Alignment / Style）：13/13 ✅
- N 组（Clipboard）：13/13 ✅
- O 组（Undo / Redo 事务）：16/16 ✅
- P 组（Persistence）：10/10 ✅
- Q 组（Performance）：10/10 ✅

**GUI 手动验收：** 2/16 完成，仍需继续实测和补齐。

- 已完成：range selection 背景连续、宽表格水平滚动条。
- 未完成：默认视觉、cell 多行高度、中文 IME、row/column handle/menu、resize/reorder、merge/split、active caret、滚动后 candidate rect、相邻 block 不重叠等手动验收项。
- S1 组（第一阶段修复）：6/6 ✅

**手动验收任务：** 2/16 (12.5%) 📋

- R 组（GUI 验收）：2/16 已验证（R-011、R-016），14 项待手动验收
...
- **Workspace 全量测试：** 已通过 ✅
- **工作区编译：** ✅
- **Postgres 集成测试：** 已编写，需 Docker 环境运行 📋

### 文档状态

- ✅ 架构文档：`doc/large-document-rich-text-architecture.md`
- ✅ 功能计划：`doc/plans/notion-table-feature-plan.md`（本文档）
- ✅ 手动验收清单：`doc/acceptance/table-manual-acceptance.md`
- ✅ 完成总结：`doc/acceptance/table-completion-summary.md`

### 下一步

1. **执行手动验收**：按照 `doc/acceptance/table-manual-acceptance.md` 逐项测试（R-001 ~ R-016）
2. **运行 Postgres 集成测试**（如环境可用）
3. **收集用户反馈**：实际使用中的体验优化

**状态：** 🎉 核心开发完成，等待手动验收和用户测试
