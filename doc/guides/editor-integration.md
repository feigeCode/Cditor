...
`SyntaxHighlightProvider`。Provider 返回原始 source 的 UTF-8 byte range、样式和当前
`languages()` 注册目录；Cditor 的语言下拉和搜索会随该目录显示。正文、Undo/Redo 与
持久化仍完全由 Cditor 管理。没有 Provider、语言不支持或 Provider 返回错误时，Cditor
会安全降级为纯文本。宿主切换主题或重新加载语言扩展时必须更新 Provider 的
`revision()`，使当前视口缓存失效。
...
Markdown Strict export 为支持语法提供语义级 round-trip；复杂表格样式、媒体属性和其他富文本专属 Block 仍应使用 `EditorDocument` JSON 保存。白板可以使用下面的 Markdown Bundle API 同时保存可见预览和可编辑源数据。

### 3.1 白板 Markdown Bundle

单文件 `export_markdown` 仍会在遇到白板时拒绝 Strict 导出，因为一个 Markdown 字符串无法同时携带预览文件和完整画板数据。需要保存白板时使用 Bundle API：

```rust
use cditor_gpui::{MarkdownBundleOptions, MarkdownExportMode};

let bundle = editor.export_markdown_bundle(
    MarkdownExportMode::Strict,
    &MarkdownBundleOptions {
        asset_directory: "设计方案.assets".to_owned(),
        preview_padding: 32,
    },
    cx,
)?;
...
for asset in &bundle.assets {
    write_asset_atomically(&asset.relative_path, &asset.bytes)?;
}
write_markdown_atomically("设计方案.md", &bundle.markdown)?;
```

白板会生成两个稳定资源：

```text
设计方案.assets/whiteboard-42.svg
设计方案.assets/whiteboard-42.cditor-board.json
```

Markdown 包含普通阅读器可显示的 SVG 图片，以及 Cditor 用于恢复可编辑白板的隐藏清单：

```markdown
<!-- cditor:whiteboard {"version":1,"block_id":42,"source":"设计方案.assets/whiteboard-42.cditor-board.json","preview":"设计方案.assets/whiteboard-42.svg","sha256":"..."} -->
![Whiteboard](<设计方案.assets/whiteboard-42.svg>)
```

宿主必须先原子写入 `bundle.assets`，最后再替换 Markdown 文件，避免 Markdown 引用了尚未落盘的资源。资源路径只能是规范化的相对路径，不能包含绝对路径、反斜杠或 `..`。

重新导入时提供资源解析器：

```rust
use cditor_gpui::{MarkdownAssetError, MarkdownAssetResolver};

struct NoteAssets {
    note_directory: std::path::PathBuf,
}

impl MarkdownAssetResolver for NoteAssets {
    fn read_asset(&self, relative_path: &str) -> Result<Vec<u8>, MarkdownAssetError> {
        std::fs::read(self.note_directory.join(relative_path))
            .map_err(|error| MarkdownAssetError::new(error.to_string()))
    }
}
...
let imported = EditorDocument::from_markdown_bundle_with_report(
    "document-1",
    source,
    &NoteAssets { note_directory },
)?;
```

导入会验证 Scene JSON 的 SHA-256。源文件缺失、损坏、哈希不匹配或路径不安全时，文档返回 `SourceOnly`，不会用空白画板覆盖原始内容。SVG 预览缺失时仍可从 Scene JSON 恢复可编辑白板，但 compatibility 为 `EditableWithNormalization` 并返回 Warning，宿主可以在下次保存时重新生成预览。
