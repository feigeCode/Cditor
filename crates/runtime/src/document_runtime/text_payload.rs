...
pub(super) fn toggle_mark_for_range(
    source: &[InlineSpan],
    range: Range<usize>,
    mark: InlineMark,
) -> Vec<InlineSpan> {
    ...
    for span in source {
        ...
        if span_range.start < range.end && range.start < span_range.end {
            ...
            every_selected_span_has_mark &= span.marks.contains(&mark);
        }
    }
    let remove_mark = saw_selected_text && every_selected_span_has_mark;
    ...
    for span in source {
        ...
        let mut selected_marks = span.marks.clone();
        if remove_mark {
            selected_marks.retain(|candidate| candidate != &mark);
        } else if !selected_marks.contains(&mark) {
...
mod tests {
    ...
    #[test]
    fn toggling_the_same_mark_twice_removes_it_and_merges_plain_spans() {
        let source = vec![InlineSpan::plain("hello")];

        let marked = toggle_mark_for_range(&source, 1..4, InlineMark::Bold);
        assert_eq!(
            marked,
            vec![
                InlineSpan::plain("h"),
                span("ell", vec![InlineMark::Bold]),
                InlineSpan::plain("o"),
            ]
        );

        let unmarked = toggle_mark_for_range(&marked, 1..4, InlineMark::Bold);
        assert_eq!(unmarked, vec![InlineSpan::plain("hello")]);
    }

    #[test]
    fn toggling_a_mark_preserves_other_marks() {
        let color = InlineMark::Color("#e03e3e".to_owned());
        let source = vec![span("styled", vec![InlineMark::Italic, color.clone()])];

        let marked = toggle_mark_for_range(&source, 0..6, InlineMark::Bold);
        assert_eq!(
            marked,
            vec![span(
                "styled",
                vec![InlineMark::Italic, color.clone(), InlineMark::Bold],
            )]
        );

        let unmarked = toggle_mark_for_range(&marked, 0..6, InlineMark::Bold);
        assert_eq!(
            unmarked,
            vec![span("styled", vec![InlineMark::Italic, color])]
        );
    }

    #[test]
    fn toggling_across_spans_adds_then_removes_the_mark_for_the_whole_range() {
        let color = InlineMark::Color("#e03e3e".to_owned());
        let source = vec![
            span("ab", vec![InlineMark::Italic]),
            span("cd", vec![InlineMark::Bold, color.clone()]),
            span("ef", vec![InlineMark::Bold]),
        ];

        let marked = toggle_mark_for_range(&source, 1..5, InlineMark::Bold);
        assert_eq!(
            marked,
            vec![
                span("a", vec![InlineMark::Italic]),
                span("b", vec![InlineMark::Italic, InlineMark::Bold]),
                span("cd", vec![InlineMark::Bold, color.clone()]),
                span("ef", vec![InlineMark::Bold]),
            ]
        );

        let unmarked = toggle_mark_for_range(&marked, 1..5, InlineMark::Bold);
        assert_eq!(
            unmarked,
            vec![
                span("ab", vec![InlineMark::Italic]),
