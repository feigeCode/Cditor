...
impl DocumentRuntime {
    ...
    fn restore_snapshot_table_focus(
        ...
        focused.offset = previous_char_boundary(&text, focused.offset.min(text.len()));
        focused.selected_range_start =
            previous_char_boundary(&text, focused.selected_range_start.min(text.len()));
        focused.selected_range_end =
            previous_char_boundary(&text, focused.selected_range_end.min(text.len()));
        if let Some(start) = focused.marked_range_start {
            focused.marked_range_start = Some(previous_char_boundary(&text, start.min(text.len())));
        }
        if let Some(end) = focused.marked_range_end {
            focused.marked_range_end = Some(previous_char_boundary(&text, end.min(text.len())));
        }
